...
export default function FavoritesScreen({ navigation }: { navigation: any }) {
  ...
  const benefits = [
    { title: 'Price Alerts', desc: 'Get notified when prices drop', icon: 'notifications-outline', color: '#3b82f6', bg: 'rgba(59, 130, 246, 0.1)' },
    { title: 'Compare', desc: 'Easily compare amenities', icon: 'git-compare-outline', color: '#10b981', bg: 'rgba(16, 185, 129, 0.1)' },
    { title: 'Fast Booking', desc: 'Access top choices instantly', icon: 'flash-outline', color: '#f59e0b', bg: 'rgba(245, 158, 11, 0.1)' }
  ];
  ...
  const collections = [
    ...
    { title: 'Student Hubs', count: '89 Places', img: 'https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=400' }
  ];

  return (
    <SafeAreaView className={`flex-1 ${bgStyles}`} edges={['top', 'left', 'right']}>
      {/* HEADER */}
      <View className={`px-5 pt-6 pb-2 flex-row justify-between items-start border-b ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
        <View>
          ...
          <View className="flex-row items-center">
            ...
            <Text className={`text-[11px] font-medium ml-1.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              Places you love, saved for later
            </Text>
          </View>
        </View>
        <TouchableOpacity
          className={`w-10 h-10 rounded-full items-center justify-center shadow-sm ${isDark ? 'bg-slate-900' : 'bg-blue-50'}`}
          style={isDark ? { shadowColor: '#020617', shadowOpacity: 0.15, shadowRadius: 3 } : { shadowColor: '#3b82f6', shadowOpacity: 0.1, shadowRadius: 3 }}
        >
      ...
      {loading ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
          ...
          {/* Main Empty Section */}
          <LinearGradient
            colors={isDark ? ['rgba(30, 58, 138, 0.1)', 'transparent'] : ['rgba(239, 246, 255, 0.5)', 'transparent']}
            className="pb-8 pt-4"
          >
            {/* Illustration */}
            ...
            <View className="px-8 items-center mb-4">
              ...
              <TouchableOpacity 
                ...
                className="mt-6 bg-[#1d4ed8] px-8 py-3.5 rounded-full flex-row items-center justify-center shadow-lg"
                style={{ shadowColor: '#3b82f6', shadowOpacity: 0.3, shadowRadius: 8, shadowOffset: { width: 0, height: 4 } }}
              >
          ...
          <View
            className="h-2"
            style={{ backgroundColor: isDark ? 'rgba(15, 23, 42, 0.8)' : '#f8fafc' }}
          />

          {/* Benefits of Saving */}
          <View className={`py-6 px-5 border-b ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
            ...
            >
              {benefits.map((item, idx) => (
                <View key={idx} className={`w-[155px] border rounded-2xl p-4 shadow-sm ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                  <View style={{ backgroundColor: item.bg }} className="w-8 h-8 rounded-full items-center justify-center mb-3">
                    <Ionicons name={item.icon as any} size={16} color={item.color} />
                  </View>
                  <Text className={`text-[12px] font-black mb-1 ${isDark ? 'text-white' : 'text-slate-800'}`}>{item.title}</Text>
                  <Text className={`text-[10px] font-medium leading-3 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{item.desc}</Text>
                </View>
              ))}
            </ScrollView>
          </View>
          ...
          {/* Curated Collections */}
          <View
            className={`py-6 px-5 border-b ${isDark ? 'border-slate-800' : 'border-slate-100'}`}
            style={{ backgroundColor: isDark ? 'rgba(15, 23, 42, 0.4)' : '#f8fafc' }}
          >
          ...
          >
            ...
            <View className="flex-row flex-wrap justify-between gap-y-3">
              {popularSearches.map((item, idx) => (
                <TouchableOpacity 
                  key={idx} 
                  className={`w-[48%] border rounded-full px-4 py-3 flex-row items-center shadow-sm ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}
                >
                  <Ionicons name={item.icon as any} size={14} color="#3b82f6" />
                  <Text className={`text-[10px] font-bold ml-2 flex-1 ${isDark ? 'text-slate-200' : 'text-slate-700'}`} numberOfLines={1}>{item.title}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
